...
from eval.s07.cwe_tree_parser import CWEParser, get_cwe_tree

# get_cwe_tree resolve CWE_XML_PATH -> data/cwec*.xml -> MITRE e normaliza a
# chave sozinho, entao o loader local e os .replace("CWE-") sairam daqui.
_get_tree = get_cwe_tree


def cwe_lookup(cwe_id: str) -> dict:
    """Returns description + parents + children + view 1003 status."""
    tree = get_cwe_tree()
    cwe = CWEParser._key(cwe_id)
    if tree is None or cwe not in tree.cwe_map:
...
def cwe_tree_path(cwe_id: str) -> dict:
    ...
    tree = get_cwe_tree()
    cwe = CWEParser._key(cwe_id)
    ancestors = tree.get_ancestors(cwe) if tree else []
...
def cwe_view_filter(cwe_id: str, view: str = "1003") -> dict:
    ...
    cwe = CWEParser._key(cwe_id)
    if view == "1003":
        in_view = bool(tree and tree.in_view_1003(cwe))
        return {"cwe_id": f"CWE-{cwe}", "in_view": in_view, "view": view}
